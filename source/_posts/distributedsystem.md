...
There are two kinds of checkpoint messages: "set" and "get". "set" simply means the replica records its database into a checkpoint (JSON data) and store it locally; while "get" simply means one replica get checkpoint data from another replica and restore data from it.
